...
describe('History Group Preservation Property Tests', () => {
  ...
  it('Property 6.1: Multiple history groups are preserved as separate entries', async () => {
    await fc.assert(
      fc.asyncProperty(fc.array(historyGroupArbitrary, { minLength: 2, maxLength: 10 }), async (historyGroupsData) => {
        ...
        if (retrievedGroups.length !== historyGroupsData.length) {
          throw new Error(`Expected ${historyGroupsData.length} history groups, got ${retrievedGroups.length}`);
        }
        ...
        for (const createdGroup of createdGroups) {
          ...
          if (!found) {
            throw new Error(`History group ${createdGroup.id} was not preserved in storage`);
          }
    ...
    await fc.assert(
      fc.asyncProperty(fc.array(historyGroupArbitrary, { minLength: 2, maxLength: 10 }), async (historyGroupsData) => {
        ...
        if (timestamps.length !== historyGroupsData.length) {
          throw new Error(`Expected ${historyGroupsData.length} timestamps, got ${timestamps.length}`);
        }
        ...
        for (const timestamp of timestamps) {
          if (!originalTimestamps.includes(timestamp)) {
            throw new Error(`Timestamp ${timestamp} not found in original timestamps`);
          }
        }
  ...
  it('Property 6.3: History groups remain separate (no merging)', async () => {
    await fc.assert(
      fc.asyncProperty(fc.array(historyGroupArbitrary, { minLength: 2, maxLength: 5 }), async (historyGroupsData) => {
        // Clear storage at the start of each iteration
        await tabGroupsStorage.setValue({});
        ...
        const retrievedTotalTabs = retrievedGroups.reduce((sum, group) => sum + group.tabs.length, 0);

        // Verify total tab count matches (no tabs lost or duplicated)
        if (retrievedTotalTabs !== totalTabs) {
          throw new Error(`Expected ${totalTabs} total tabs, got ${retrievedTotalTabs}`);
        }
  ...
  it('Property 6.4: History groups persist across storage operations', async () => {
    await fc.assert(
      fc.asyncProperty(fc.array(historyGroupArbitrary, { minLength: 1, maxLength: 5 }), async (historyGroupsData) => {
        // Clear storage at the start of each iteration
        await tabGroupsStorage.setValue({});
