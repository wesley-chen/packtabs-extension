    ...
    await fc.assert(
      fc.asyncProperty(historyGroupArbitrary, groupNameArbitrary, async (historyGroup, newName) => {
        ...
        if (convertedGroup.id !== historyGroup.id) {
          throw new Error(`Converted group ID mismatch: expected ${historyGroup.id}, got ${convertedGroup.id}`);
        }
        ...
        if (convertedGroup.isHistory) {
          throw new Error(`Converted group should have isHistory=false, got ${convertedGroup.isHistory}`);
        }
    ...
    await fc.assert(
      fc.asyncProperty(historyGroupArbitrary, groupNameArbitrary, async (historyGroup, newName) => {
        ...
        if (finalTotalCount !== initialTotalCount) {
          throw new Error(`Total group count changed: expected ${initialTotalCount}, got ${finalTotalCount}`);
        }
      ...
      fc.asyncProperty(
        ...
        async (historyGroups, names) => {
          ...
          for (let i = 0; i < numToConvert; i++) {
            ...
            if (convertedGroup.isHistory) {
              throw new Error(`Converted group ${i} should have isHistory=false`);
            }
          }
          ...
          for (let i = numToConvert; i < testGroups.length; i++) {
            ...
            if (!historyGroup.isHistory) {
              throw new Error(`History group ${i} should have isHistory=true`);
            }
          }
    ...
    await fc.assert(
      fc.asyncProperty(historyGroupArbitrary, groupNameArbitrary, async (historyGroup, newName) => {
        ...
        if (initialSelectedGroup.id !== historyGroup.id) {
          throw new Error(`Selected group ID mismatch: expected ${historyGroup.id}, got ${initialSelectedGroup.id}`);
        }
        ...
        if (finalSelectedGroup.isHistory) {
          throw new Error(`Selected group should have isHistory=false after conversion`);
        }
