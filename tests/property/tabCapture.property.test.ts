        ...
        async (browserTabs) => {
          ...
          for (let i = 0; i < captured.length; i++) {
            ...
            if (capturedTab.title !== expectedTitle) {
              throw new Error(
                `Tab ${i} title mismatch: expected ${expectedTitle}, got ${capturedTab.title}`
              );
            }
            ...
            // Each captured tab should have a unique ID
            if (!capturedTab.id || typeof capturedTab.id !== 'string') {
              throw new Error(`Tab ${i} missing valid ID`);
            }
          }
      ...
      fc.asyncProperty(
        ...
        async (browserTabs) => {
          ...
          if (uniqueIds.size !== ids.length) {
            throw new Error(
              `Duplicate IDs found: expected ${ids.length} unique IDs, got ${uniqueIds.size}`
            );
          }
