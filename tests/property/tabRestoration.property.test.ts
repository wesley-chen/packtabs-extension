...
describe('Tab Restoration Property Tests', () => {
      ...
      fc.asyncProperty(
        ...
        async (tabs) => {
          ...
          if (createdTabs.length !== tabs.length) {
            throw new Error(
              `Tab count mismatch: expected ${tabs.length} tabs to be opened, got ${createdTabs.length}`
            );
          }
        ...
        async (tabs) => {
          ...
          for (let i = 0; i < tabs.length; i++) {
            ...
            if (createdUrl !== originalUrl) {
              throw new Error(
                `Tab ${i} URL mismatch: expected ${originalUrl}, got ${createdUrl}`
              );
            }
          }
  ...
  it('Property 11.4: Maintains tab order during restoration', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(tabItemArbitrary, { minLength: 2, maxLength: 20 }),
        async (tabs) => {
          ...
          for (let i = 0; i < tabs.length; i++) {
            if (createdUrls[i] !== tabs[i].url) {
              throw new Error(
                `Tab order mismatch at position ${i}: expected ${tabs[i].url}, got ${createdUrls[i]}`
              );
            }
          }
