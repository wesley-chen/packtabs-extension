...
describe('Automatic Tab Cleanup Property Tests', () => {
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (browserTabs) => {
          ...
          const expectedTabIds = browserTabs
            .map(tab => tab.id)
          ...
          if (removedTabIds.length === 0 && expectedTabIds.length > 0) {
            throw new Error(
              `Expected tabs to be closed, but tabs.remove was not called`
            );
          }
          ...
          if (removedTabIds.length !== expectedTabIds.length) {
            throw new Error(
              `Expected ${expectedTabIds.length} tabs to be closed, got ${removedTabIds.length}`
            );
          }
  ...
  it('Property 3.2: Tab cleanup handles empty tab list gracefully', async () => {
    ...
    if (removeSpy.mock.calls.length > 0) {
      ...
      if (Array.isArray(callArgs) && callArgs.length > 0) {
        throw new Error(
          `tabs.remove should not be called with tab IDs when there are no tabs`
        );
      }
    }
  });
      ...
      fc.asyncProperty(
        ...
        async (browserTabs) => {
          ...
          if (removeSpy.mock.calls.length !== 1) {
            throw new Error(
              `Expected tabs.remove to be called once, got ${removeSpy.mock.calls.length} calls`
            );
          }
  ...
  it('Property 3.4: Tab cleanup preserves tab order in removal', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(browserTabArbitrary, { minLength: 2, maxLength: 10 }),
        async (browserTabs) => {
          // Setup: Mock browser APIs
          (global as any).browser = (global as any).browser || {};
          (global as any).browser.windows = (global as any).browser.windows || {};
          ...
          if (removedTabIds.length !== expectedTabIds.length) {
            throw new Error(
              `Tab count mismatch: expected ${expectedTabIds.length}, got ${removedTabIds.length}`
            );
          }
          ...
          for (const expectedId of expectedTabIds) {
            if (!removedTabIds.includes(expectedId)) {
              throw new Error(
                `Expected tab ID ${expectedId} not found in removed tabs`
              );
            }
          }
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ currentWindowTabs, otherWindowTabs }) => {
          ...
          const otherWindowTabIds = otherWindowTabs
            .map(tab => tab.id)
            .filter((id): id is number => id !== undefined);
          ...
          for (const removedId of removedTabIds) {
            if (!currentWindowTabIds.includes(removedId)) {
              throw new Error(
                `Tab ${removedId} from other window was incorrectly closed`
              );
            }
          }
          ...
          for (const otherId of otherWindowTabIds) {
            if (removedTabIds.includes(otherId)) {
              throw new Error(
                `Tab ${otherId} from other window should not be closed`
              );
            }
          }
