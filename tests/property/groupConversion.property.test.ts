      ...
      fc.asyncProperty(
        ...
        async (historyGroup, newName) => {
          ...
          if (convertedGroup.name !== newName) {
            throw new Error(
              `Expected group name to be "${newName}", got "${convertedGroup.name}"`
            );
          }
          ...
          if (convertedGroup.isHistory) {
            throw new Error(
              `Expected isHistory to be false after conversion, got ${convertedGroup.isHistory}`
            );
          }
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (historyGroup, newName) => {
          ...
          if (!convertedGroup) {
            throw new Error(
              `Group with ID ${originalId} not found after conversion`
            );
          }

          if (convertedGroup.id !== originalId) {
            throw new Error(
              `Group ID changed during conversion: expected ${originalId}, got ${convertedGroup.id}`
            );
          }
      ...
      fc.asyncProperty(
        ...
        async (historyGroups, names) => {
          ...
          if (groups.length !== testGroups.length) {
            throw new Error(
              `Expected ${testGroups.length} groups initially, got ${groups.length}`
            );
          }
          ...
          if (groups.length !== testGroups.length) {
            throw new Error(
              `Expected ${testGroups.length} groups after conversion, got ${groups.length}`
            );
          }
          ...
          if (convertedGroup.name !== testNames[0]) {
            throw new Error(
              `Expected converted group name to be "${testNames[0]}", got "${convertedGroup.name}"`
            );
          }
          ...
          for (let i = 1; i < testGroups.length; i++) {
            ...
            if (otherGroup.name !== null) {
              throw new Error(
                `Group ${i} should still have null name, got "${otherGroup.name}"`
              );
            }

            if (!otherGroup.isHistory) {
              throw new Error(
                `Group ${i} should still have isHistory=true, got ${otherGroup.isHistory}`
              );
            }
          }
