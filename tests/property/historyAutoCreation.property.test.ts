  ...
  it('Property 5.1: Creates History Tab Group on browser close with all tabs', async () => {
    await fc.assert(
      fc.asyncProperty(fc.array(browserTabArbitrary, { minLength: 1, maxLength: 20 }), async (browserTabs) => {
        ...
        if (groups.length !== 1) {
          throw new Error(`Expected 1 history group, got ${groups.length}`);
        }
        ...
        if (!historyGroup.isHistory) {
          throw new Error(`Expected isHistory to be true, got ${historyGroup.isHistory}`);
        }

        // Verify all tabs were captured
        if (historyGroup.tabs.length !== browserTabs.length) {
          throw new Error(`Expected ${browserTabs.length} tabs in history group, got ${historyGroup.tabs.length}`);
        }
    ...
    await fc.assert(
      fc.asyncProperty(fc.array(browserTabArbitrary, { minLength: 1, maxLength: 10 }), async (browserTabs) => {
        ...
        if (!savedGroup.isHistory) {
          throw new Error(`History group should have isHistory=true, got ${savedGroup.isHistory}`);
        }
